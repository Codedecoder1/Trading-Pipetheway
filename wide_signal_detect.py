"""
Wide-window Stage C -- run the SMC confluence detector, walk-forward style,
against every (symbol, date) mover event in the WIDE 14-week scope
(2026-05-18 through 2026-08-24, 53 symbols after excluding the same 8
chronic movers used in the original narrowed backtest), using the
consolidated 5-minute intraday bars in wide_5min_bars.json.

This is the direct wide-window analogue of phase1_signal_detect.py --
identical methodology (STRICT/LOOSE confluence thresholds, 5min/10min/30min
walk-forward consolidation windows, bar-by-bar mover_pct recompute vs. that
day's own prev_close so a signal only counts once the intraday move has
actually crossed the >=3.0% production movers threshold at the moment it
would have fired live). Each (symbol, date) is walked forward independently;
the consolidation/POC window never spans across days.

DRY RUN / SIGNAL-ONLY: this script only detects raw confluence signals. It
does not touch contract_filters.py's Layer 1/2 gate and it never calls any
...
with open('/home/claude/smc_bot/diag/backtest/wide_mover_events_final.json') as f:
    events = json.load(f)
...
print(f"Skipped (no intraday data): {len(skipped_no_data)}")
if skipped_no_data:
    print(f"  {skipped_no_data}")
print(f"Skipped (too short for any interval): {len(skipped_too_short)}")
if skipped_too_short:
    print(f"  {skipped_too_short}")
print(f"Total raw confluence signal hits: {len(all_hits)}")

with open('/home/claude/smc_bot/diag/backtest/wide_raw_signals.json', 'w') as f:
...
by_date = Counter(h["date"] for h in all_hits)
print("\nBy symbol (top 20):", by_symbol.most_common(20))
print("By config:", dict(by_config))
print("By interval:", dict(by_interval))
print("By direction:", dict(by_direction))
print("Unique (symbol,date) with >=1 signal:", len(by_date and set((h['symbol'], h['date']) for h in all_hits)))
