...
print("")
print("=== PLACE AS A BRACKET -- all three, in order, at confirmation ===")
print("The stop and take-profit are RESTING orders. Once they're at the broker they")
print("manage the trade on their own; the Exit Monitor task only covers the gaps.")
print("")
print(f"1. ENTRY -- BUY to open")
print(f"   {entry_qty}x {args.symbol} ${args.strike} {args.direction} exp {args.expiration}")
print(f"   Order type   : limit @ ${args.ask:.2f}   (${args.ask*100*entry_qty:.2f} total, bid ${args.bid:.2f})")
print(f"   Option id    : {args.option_id}")
print("")
print(f"2. STOP-LOSS -- SELL to close, immediately after the entry fills")
print(f"   {entry_qty}x {args.symbol} ${args.strike} {args.direction} exp {args.expiration}")
print(f"   Order type   : stop-market")
print(f"   Stop trigger : ${hard_stop_price:.2f}   (entry ${args.ask:.2f} minus {_stop_pct}%, rounded to the cent)")
print(f"   Time in force: GTC")
print("")
_tp_share = f"sell {tp_qty}, keep {runner_qty} as a runner" if runner_qty > 0 else "1-lot, closes the whole position"
print(f"3. TAKE-PROFIT -- SELL to close, alongside the stop ({_tp_share})")
print(f"   {tp_qty}x {args.symbol} ${args.strike} {args.direction} exp {args.expiration}")
print(f"   Order type   : limit @ ${take_profit_price:.2f}   (entry ${args.ask:.2f} plus {_tp_pct}%)")
print(f"   Time in force: GTC")
